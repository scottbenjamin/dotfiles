function wts --description "Git worktree manager"
    set -l cmd $argv[1]

    # Capture relative path within current worktree for preserving on switch
    set -l rel_path ""
    set -l cur_toplevel (git rev-parse --show-toplevel 2>/dev/null)
    if test -n "$cur_toplevel"
        set -l cwd (pwd)
        set rel_path (string replace "$cur_toplevel" "" -- "$cwd" | string replace -r "^/" "")
    end
    ...
    if test -z "$cmd"
        ...
        or return 0
        set -l target (string split -f1 " " -- $selected)
        __wt_cd_relative $target $rel_path
        __wt_print_location
        return
    end
    ...
    switch $cmd
        ...
        case new
            set -l branch $argv[2]
            if test -z "$branch"
                echo "Usage: wts new <branch-name>"
                return 1
            end
        ...
        case '*'
            ...
            if test (count $matches) -eq 0
                ...
                or return 0
                __wt_cd_relative $selected $rel_path
            else
                __wt_cd_relative $matches[1] $rel_path
            end
            __wt_print_location
    end
end

function __wt_cd_relative --description "cd to worktree, preserving relative path if it exists"
    set -l target $argv[1]
    set -l rel $argv[2]
    if test -n "$rel" -a -d "$target/$rel"
        cd "$target/$rel"
    else
        cd $target
    end
